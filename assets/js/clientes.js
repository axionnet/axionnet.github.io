...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const elementos = {
    ...
    aviso: document.querySelector('[data-aviso-avaliacao]')
  };
  ...
  const enviarFoto = async (blob) => {
    ...
    const nomeArquivo = `public/${Date.now()}-${crypto.randomUUID()}.webp`;
  ...
  const publicar = async (dados) => {
    ...
    const resposta = await fetch(`${config.url}/rest/v1/${config.tabelaAvaliacoes}`, {
      ...
      body: JSON.stringify({ ...dados, aprovada: false })
    });
  ...
  formulario.addEventListener('submit', async (evento) => {
    ...
    try {
      ...
      fecharModal();
      if (elementos.aviso) {
        elementos.aviso.textContent = modoOnline
          ? 'Avaliação enviada com sucesso. Ela aparecerá após a aprovação da equipe.'
          : 'Avaliação salva neste navegador em modo de demonstração.';
        elementos.aviso.hidden = false;
        elementos.aviso.scrollIntoView({ behavior: 'smooth', block: 'center' });
      }
